...
from src.state import ArtifactRecord, ArtifactStatus, ArtifactType, FailureCategory, FindingSeverity
from src.tools import load_incident_artifacts, inspect_dependencies
...
def test_fixture_requirements_file_passes() -> None:
    ...
    assert not any(f.severity in (FindingSeverity.MEDIUM, FindingSeverity.HIGH) for f in result.findings)
